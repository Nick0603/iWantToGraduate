 const modules = {
    ...
    getTotalCourse :(tbody_arr,course_index) => {
        let buffer_course       = modules.getCourse(tbody_arr, course_index.buffer_course_index).map(modules.getBufferCourse);
        let semester_course     = modules.getCourse(tbody_arr, course_index.semester_course_index).map(modules.getSemesterCourse);
        let substitution_course = modules.getCourse(tbody_arr, course_index.substitution_course_index).map(modules.getSubstitutionCourse);
    ...
    collectPassedCourse : (course_index) => {
        ...
        let total_course = modules.getTotalCourse(tbody_arr,course_index);
        let passed_course = total_course.filter(modules.passCourseFilter);
        return passed_course;
    },
}

const {collectPassedCourse} = modules;
let main = () => {
